...
tf.compat.v1.flags.DEFINE_integer("emb_dim", 32, "The dimensionality of embedding (default: 2)")
...
if config.train_mode:
    with tf.compat.v1.Session(config=gpu_config) as sess:
        ...
        procedure_CIS.train_network(sess, net, train_generator, validate_generator, config.nb_epoch,
...
if config.prediction_mode or config.tune_mode:
    with tf.compat.v1.Session(config=gpu_config) as sess:
        ...
        if config.prediction_mode:
            ...
            procedure_CIS.generate_prediction_for_training(net, train_generator1, total_train_batches, config.display_step, item_dict_train,
                        rev_item_dict, output_dir + "/topN/train_recall_CIS.txt", offered_courses_all)
        ...
        if config.tune_mode:
            print("@Start tunning")
            validate_generator = utils.seq_batch_generator(validate_instances, item_dict_train, config.batch_size, False)
            procedure_CIS.tune(net, validate_generator, total_validate_batches, config.display_step, item_dict,
                        rev_item_dict, output_dir + "/topN/val_recall_CIS.txt", offered_courses_all, nb_validate, config.batch_size)
        ...
        if config.prediction_mode:
            ...
            procedure_CIS.generate_prediction(net, test_generator, total_test_batches, config.display_step, item_dict,
                        rev_item_dict, output_dir + "/topN/test_prediction_CIS.txt", offered_courses_all, output_path, output_dir2, nb_test, config.batch_size)
