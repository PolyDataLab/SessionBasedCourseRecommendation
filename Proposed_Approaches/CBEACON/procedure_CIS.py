...
def recommend_top_k(user_baskets, prob_item, offered_course_list, top_k1, item_dict, rev_item_dict):
    ...
    for item1 in prob_item.keys():
        ...
        if not utils.filtering(item1, user_baskets, offered_course_list, item_dict):
            #if course_CIS_dept_filtering(rev_item_dict[item1]):
...
def tune(net, data_generator, total_batches, display_step, item_dict, rev_item_dict, output_file, offered_courses_valid, nb_validate, batch_size):
    ...
    for batch_id, data in data_generator:
        ...
        for i, (seq_val, seq_ind) in enumerate(zip(values, indices)):
            ...
            # target_courses= data['O'][i].split(" ")
            # target_courses_CIS = course_CIS_dept(target_courses)
            # target_basket2 = []
            # for x1 in range(len(target_courses_CIS)):
            #     if(len(target_courses_CIS[x1])>0):
            #         target_basket2.append(item_dict[target_courses_CIS[x1]])
            # target_basket_CIS = target_basket2.copy()
            ...
            top_items = recommend_top_k(user_baskets, prob_item, offered_courses_valid[int(target_semester)], len(target_basket), item_dict, rev_item_dict)
            
            target_courses= data['O'][i].split(" ")
            target_courses_CIS = course_CIS_dept(target_courses)
            pred_courses = []
            for item in top_items:
                pred_courses.append(rev_item_dict[item])
    ...
    precision = precision/count2
    # recall_average = cor_preds_total/ actual_bsize_total
    print (": average precision@n"+ " for all validating batches after using filtering function: "+ str(precision))
    f.write("average precision@n"+ " for all validating batches after using filtering function: "+ str(precision))
    f1_score = f1_score/count2
    # recall_average = cor_preds_total/ actual_bsize_total
    print (": average f1_score@n"+ " for all validating batches after using filtering function: "+ str(f1_score))
    f.write("average f1_score@n"+ " for all validating batches after using filtering function: "+ str(f1_score))
    f.close()
...
def generate_prediction_for_training(net, data_generator, total_train_batches, display_step, item_dict, inv_item_dict, output_file, offered_courses_train):
    ...
    for batch_id, data in data_generator:
        ...
        for i, (seq_val, seq_ind) in enumerate(zip(values, indices)):
            ...
            top_items = recommend_top_k(user_baskets, prob_item, offered_courses_train[int(target_semester)], len(target_basket), item_dict, inv_item_dict)
            target_courses= data['O'][i].split(" ")
            target_courses_CIS = course_CIS_dept(target_courses)
            pred_courses = []
            for item in top_items:
                pred_courses.append(inv_item_dict[item])
    ...
    precision = precision/count2
    # recall_average = cor_preds_total/ actual_bsize_total
    print (": average precision@n"+ " for all training batches after using filtering function: "+ str(precision))
    f.write("average precision@n"+ " for all training batches after using filtering function: "+ str(precision))
    f1_score = f1_score/count2
    # recall_average = cor_preds_total/ actual_bsize_total
    print (": average f1_score@n"+ " for all training batches after using filtering function: "+ str(f1_score))
    f.write("average f1_score@n"+ " for all training batches after using filtering function: "+ str(f1_score))
    f.close()
...
def calculate_mse_for_course_allocation(term_dict, term_dict_predicted, term_dict_predicted_true, term_dict_predicted_false, count_total_course, item_dict, count_course_avg, course_sd_main, course_number_terms, term_dict_all_prior, output_dir):
    ...
    #count1= 0
    count2 = 0
    output_path1= output_dir+ "/test_course_allocation_CIS_v2.txt"
    ...
    course_allocation_actual_predicted.to_csv(output_dir+'/course_allocation_actual_predicted_updated_new_CIS_v2.csv')
...
def generate_prediction(net, data_generator, total_test_batches, display_step, item_dict, inv_item_dict, output_file, offered_courses_test, output_path, output_dir2, nb_test, batch_size):
    ...
    for batch_id, data in data_generator:
        ...
        for i, (seq_val, seq_ind) in enumerate(zip(values, indices)):
            ...
            top_items = recommend_top_k(user_baskets, prob_item, offered_courses_test[int(target_semester)], len(target_basket2), item_dict, inv_item_dict)
            #print(top_items)
            #f1.write(str(user)+ ": ")
            ...
            if len(target_courses_CIS)>0:
                ...
                f1_score_2 += f1_score_temp
                count1+= 1
                term_dict = calculate_term_dict_2(term_dict, int(target_semester), target_courses_CIS, inv_item_dict)
                #course allocation for predicted courses
                term_dict_predicted = calculate_term_dict_2(term_dict_predicted, int(target_semester), pred_courses_CIS, inv_item_dict)
                term_dict_predicted_true = calculate_term_dict_true(term_dict_predicted_true, int(target_semester), target_courses_CIS, pred_courses_CIS, inv_item_dict)
                term_dict_predicted_false = calculate_term_dict_false(term_dict_predicted_false, int(target_semester), target_courses_CIS, pred_courses_CIS, inv_item_dict)
    ...
    precision = precision/count2
    # recall_average = cor_preds_total/ actual_bsize_total
    print (": average precision@n"+ " for all testing batches after using filtering function: "+ str(precision))
    f.write("average precision@n"+ " for all testing batches after using filtering function: "+ str(precision))
    f1_score = f1_score/count2
    # recall_average = cor_preds_total/ actual_bsize_total
    print (": average f1_score@n"+ " for all testing batches after using filtering function: "+ str(f1_score))
    f.write("average f1_score@n"+ " for all testing batches after using filtering function: "+ str(f1_score))
