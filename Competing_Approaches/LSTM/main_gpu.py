...
tf.compat.v1.flags.DEFINE_integer("hidden_layers", 2, "The number of hidden layers of RNN (default: 3)")
tf.compat.v1.flags.DEFINE_integer("nb_hop", 1, "The number of neighbor hops  (default: 1)")
tf.compat.v1.flags.DEFINE_float("alpha", 0, "The reguralized hyper-parameter (default: 0.5)")
tf.compat.v1.flags.DEFINE_integer("matrix_type", 0, "The type of adjacency matrix (0=zero,1=real,default:1)") # we do not use any adjacency matrix here
...
if config.train_mode:
    with tf.compat.v1.Session(config=gpu_config) as sess:
        ...
        # train_generator = utils.seq_batch_generator_differently(training_instances, item_dict, config.batch_size)
        # validate_generator = utils.seq_batch_generator_differently(validate_instances, item_dict, config.batch_size, False)
        # test_generator = utils.seq_batch_generator_differently(testing_instances, item_dict, config.batch_size, False)
        
        # Initialize the network
        print(" + Initialize the network")
        net = models.LSTM_model(sess, config.emb_dim, config.rnn_unit, config.hidden_layers, config.alpha, MAX_SEQ_LENGTH, item_probs, adj_matrix, config.top_k, 
...
if config.prediction_mode or config.tune_mode:
    with tf.compat.v1.Session(config=gpu_config) as sess:
        ...
        if config.prediction_mode:
            ...
            train_generator1 = utils.seq_batch_generator(training_instances, item_dict, config.batch_size, False)
            #train_generator1 = utils.seq_batch_generator_differently(training_instances, item_dict, config.batch_size, False)
            procedure.generate_prediction_for_training(net, train_generator1, total_train_batches, config.display_step, item_dict,
        ...
        if config.tune_mode:
            ...
            procedure.tune(net, validate_generator, total_validate_batches, config.display_step, item_dict,
                        rev_item_dict, output_dir + "/topN/val_recall.txt", offered_courses_all)
        ...
        if config.prediction_mode:
            ...
            procedure.generate_prediction(net, test_generator, total_test_batches, config.display_step, item_dict,
                        rev_item_dict, output_dir + "/topN/prediction.txt", offered_courses_all, output_path, output_dir2)
