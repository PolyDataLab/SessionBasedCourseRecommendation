...
def train_network(sess, net, train_generator, validate_generator, nb_epoch, 
    ...
    for epoch in range(0, nb_epoch):
        ...
        for batch_id, data in train_generator:
            ...
            loss, recall, summary, values, indices = net.train_batch(data['S'], data['L'], data['Y'])
...
#recommend top_k items for testing basket using filtering function
def recommend_top_k(user_baskets, prob_item, offered_course_list, top_k1, item_dict):
    ...
    for item1 in prob_item.keys():
        ...
        if not filtering(item1, user_baskets, offered_course_list, item_dict):
...
def tune(net, data_generator, total_batches, display_step, item_dict, rev_item_dict, output_file, offered_courses_valid):
...
def generate_prediction_for_training(net, data_generator, total_train_batches, display_step, item_dict, inv_item_dict, output_file, offered_courses_train):
    ...
    f.close()
# calculating term dictionary wher key = semester, value = dictionary with course and number of actual enrollments in that course
...
def generate_prediction(net, data_generator, total_test_batches, display_step, item_dict, inv_item_dict, output_file, offered_courses_test, output_path, output_dir):
    ...
    for batch_id, data in data_generator:
        ...
        for i, (seq_val, seq_ind) in enumerate(zip(values, indices)):
            ...
            target_basket3 = target_basket.copy()
            ...
            for x1 in range(len(target_basket)):
                if(len(target_basket[x1])>0):
                    #target_basket[x1]= item_dict[target_basket[x1]]
                    target_basket[index1]= item_dict[target_basket[x1]]
                    index1 +=1
            ...
            #recall_temp =0.0
            top_items = recommend_top_k(user_baskets, prob_item, offered_courses_test[int(target_semester)], len(target_basket), item_dict)
            #print(top_items)
            #f1.write(str(user)+ ": ")
            rec_courses = []
            ...
            for item3 in top_items:
                ...
                rec_courses.append(inv_item_dict[item3])
            f1.write("\n")
            # target_courses= data['O'][i].split(" ")
            # target_courses_CIS = course_CIS_dept(target_courses)
            # pred_courses = []
            ...
            rel_rec = len((set(top_items) & set(target_basket)))
            row = [t_length, target_basket3, rec_courses, rel_rec, recall_temp, target_semester]
            rec_info.append(row)
            ...
            term_dict_predicted_true = calculate_term_dict_true(term_dict_predicted_true, int(target_semester), target_basket, top_items, inv_item_dict)
            term_dict_predicted_false = calculate_term_dict_false(term_dict_predicted_false, int(target_semester), target_basket, top_items, inv_item_dict)
            # recall_1 += recall_temp
            # #recall_1 += recall_cal(top_items, target_basket) 
            # count1 += 1
        recall_main= recall_1/ count1
    ...
    test_rec_info.to_json('./Others/LSTM/LSTM_test_rec_info.json', orient='records', lines=True)
    test_rec_info.to_csv('./Others/LSTM/LSTM_test_rec_info.csv')
    count_total_course = {}
